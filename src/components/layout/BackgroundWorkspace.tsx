...
import { Cloud, Database, Server, Activity, Zap } from "lucide-react";
...
const BackgroundWorkspace = () => {
  ...
  return (
    <div className="ide-workspace-bg pointer-events-none select-none">
      <div className="ide-mesh-gradient" />
      <div className="ide-grid" />

      {/* 1. IDE Code Layer (Blurred Scrolling Code) - DISABLED ON MOBILE */}
      {!isMobile && (
        <div className="absolute inset-0 opacity-[0.05] blur-[3px] overflow-hidden will-change-transform">
      ...
      {/* 2. Terminal Layer (Drifting Logs) - DISABLED ON MOBILE */}
      {!isMobile && (
        <div className="absolute inset-0 overflow-hidden">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="animate-drift-logs absolute left-[10%] font-mono text-[9px] text-muted-foreground/30 will-change-transform"
              style={{ 
                top: `${20 + i * 25}%`, 
      ...
      {/* 3. Architecture / Network Layer (Glowing SVG Lines) - SIMPLIFIED ON MOBILE */}
      <svg className="absolute inset-0 w-full h-full opacity-[0.05]">
        <path
          ...
          stroke="currentColor"
          strokeWidth="1"
          className="text-primary animate-pulse-line will-change-[stroke-dashoffset]"
        />
        {!isMobile && (
          <>
            <path
              ...
              stroke="currentColor"
              strokeWidth="1"
              className="text-accent animate-pulse-line will-change-[stroke-dashoffset]"
      ...
      {/* 4. Cloud / Infra Layer (Floating Icons) - SIMPLIFIED ON MOBILE */}
      <div className="absolute inset-0 overflow-hidden">
        <motion.div
          animate={!isMobile ? { y: [0, -20, 0] } : {}}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-[12%] right-[10%] md:right-[18%] text-primary/10 md:text-primary/20 flex flex-col items-center will-change-transform"
        >
          <div className="relative">
            <Cloud size={isMobile ? 80 : 140} strokeWidth={1} />
            <div className="absolute top-4 right-4 w-1 h-1 rounded-full bg-green-500/40 animate-pulse" />
          </div>
          <span className="font-mono text-[6px] md:text-[8px] text-primary/40 mt-2 uppercase tracking-widest bg-primary/5 px-2 py-0.5 rounded border border-primary/10">
            us-east-1
          </span>
        </motion.div>
        ...
        {!isMobile && (
          <>
            <motion.div
              ...
              className="absolute bottom-[18%] left-[8%] text-accent/20 flex flex-col items-center will-change-transform"
            >
              <div className="relative">
            ...
            <motion.div
              ...
              className="absolute top-[55%] right-[12%] text-green-500/20 flex flex-col items-center will-change-transform"
            >
              <div className="relative">
...
export default memo(BackgroundWorkspace);